...
"""
This script is used to 预处理MOD13A3数据集
...
# 迭代每月hdf文件处理
pbar = tqdm(range(month_count), desc='MOD13A3-预处理', ncols=120, colour='blue')
for cur_month_count in pbar:
    ...
    cur_out_path = os.path.join(out_dir,
                                'MOD13A3_{}_{:02}{:02}.tif'.format(cur_date.year, cur_date.month, cur_date.day))
    ...
    for ix, cur_path in enumerate(cur_paths):
        ...
        # cur_out_path = os.path.join(temp_dir, cur_out_filename)
    ...
    try:
        ...
        pbar.write('\n{}-{:02}-{:02}: 发生异常终止当前日期的处理并退出程序(error: {})'.format(cur_date.year, cur_date.month,
                                                                                   cur_date.day, e))
        exit(1)  # 退出程序
    finally:
        # 删除临时文件和释放资源
